    ...
    try {
        ...
        while(sender.is_alive() && receiver.is_alive()){
            ...
            if (receiver_queue.try_pop(gs)){
                ...
                for (int i=0; i<map.platforms_len;i++){
                    if(i%32 == 0){
                        renderer.Copy(platformTexture, SDL_Rect{0, 11*7, 16, 8}, SDL_Rect{map.platforms[i].x, map.platforms[i].y-16, 32, 16});
                    }
                }
                ...
                for (int i=0; i < gs.ducks.size(); i++) {
                    ...
                    if (gs.ducks[i].jumping){
                        renderer.Copy(duckTexture, SDL_Rect{1*32+1, 44, 32, 32}, SDL_Rect{gs.ducks[i].x-16, gs.ducks[i].y-64, 64, 64}, 0, NullOpt, dir[i]);
                    } else if (gs.ducks[i].falling){
                        int ai = 10;
                        if (dir[i]){
                            ai = 22;
                        }
                        renderer.Copy(duckTexture, SDL_Rect{3*32+1, 44, 32, 32}, SDL_Rect{gs.ducks[i].x-16, gs.ducks[i].y-64, 64, 64}, 0, NullOpt, dir[i]);
                        renderer.Copy(wingTexture, SDL_Rect{ja[i]*16, 32, 16, 16}, SDL_Rect{gs.ducks[i].x-16+ai, gs.ducks[i].y+15-64, 32, 32}, 0, NullOpt, dir[i]);
                    } else if (gs.ducks[i].is_moving_right ^ gs.ducks[i].is_moving_left){
                        int ai = 10;
                        if (dir[i]){
                            ai = 22;
                        }
                        renderer.Copy(duckTexture, SDL_Rect{j[i]*32+1, 8, 32, 32}, SDL_Rect{gs.ducks[i].x-16, gs.ducks[i].y-64, 64, 64}, 0, NullOpt, dir[i]);
                        renderer.Copy(wingTexture, SDL_Rect{16, 0, 16, 16}, SDL_Rect{gs.ducks[i].x-16+ai, gs.ducks[i].y+25-64, 32, 32}, 0, NullOpt, dir[i]);
                    } else {
                        renderer.Copy(duckTexture, SDL_Rect{1, 8, 32, 32}, SDL_Rect{gs.ducks[i].x-16, gs.ducks[i].y-64, 64, 64}, 0, NullOpt, dir[i]);
                        int ai = 10;
                        if (dir[i]){
                            ai = 22;
                        }
                        renderer.Copy(wingTexture, SDL_Rect{0, 0, 16, 16}, SDL_Rect{gs.ducks[i].x-16+ai, gs.ducks[i].y+25-64, 32, 32}, 0, NullOpt, dir[i]);
                    }
