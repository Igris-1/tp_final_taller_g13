...
void GearView::draw_accessories(duck_DTO duck, bool facing_direction, int inclination, int x, int y, std::vector<int>* a_f){
	...
	if (duck.helmet_equipped){
        ...
        if (duck.crouching){
            ...
            renderer.Copy(accessories_textures[0], SDL_Rect{8, 10, 16, 16},
                      SDL_Rect{x + x_sum_value + facing_direction*x_mult_value, y + y_sum_value, 28, 28}, 0, NullOpt,
                      facing_direction);
        } else
		if (inclination != 0){
			renderer.Copy(accessories_textures[0], SDL_Rect{8, 10, 16, 16},
...
void GearView::draw_held_gear(bool& facing_direction, duck_DTO& duck, int inclination, int x_sum_value, int y_sum_value) {
