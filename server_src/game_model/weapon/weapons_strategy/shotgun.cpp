...
std::vector<std::shared_ptr<BulletInterface>> Shotgun::fire(std::shared_ptr<Duck> duck_trigger,
    ...
    if (this->fire_rate == 0) {
        bullets.push_back(std::make_shared<Pellet>(duck_trigger->get_id(), x_position, y_position,
                                                   x_direction, y_direction, TRAVEL_DISTANCE_SHOTGUN));
        bullets.push_back(std::make_shared<Pellet>(duck_trigger->get_id(), x_position, y_position-1,
                                                   x_direction, y_direction, TRAVEL_DISTANCE_SHOTGUN));

        bullets.push_back(std::make_shared<Pellet>(duck_trigger->get_id(), x_position, y_position+1,
                                                   x_direction, y_direction+1, TRAVEL_DISTANCE_SHOTGUN));

        bullets.push_back(std::make_shared<Pellet>(duck_trigger->get_id(), x_position, y_position-1,
                                                   x_direction, y_direction-1, TRAVEL_DISTANCE_SHOTGUN));

        bullets.push_back(std::make_shared<Pellet>(duck_trigger->get_id(), x_position, y_position-2,
                                                   x_direction, y_direction-2, TRAVEL_DISTANCE_SHOTGUN));
